...
This final query is used to find how quickly the company is making sales. Firstly the `WITH` clause is to create a common table expression called `sales_rate`.Then from the date table, this statement will compute the year and the difference between a string operator that combines the year, month, day and timestamp columns as a timestamp datatype and an offset of that string operator created using the `LEAD` function. `LEAD` will create a column where the value for each row are the same of the string operator in the following row creating an offset. The difference between the string operator and the offset is saved as the 'time_taken'.    
